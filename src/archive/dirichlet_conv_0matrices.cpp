...
// spaces and matrices with "0" in variable name
    ...
    for (int ref_step=0; ref_step<=ref_steps; ref_step++) {
        ...
        *D0T_n *= -1.;
        ...
        for (double t = dt ; t < tmax+dt ; t+=dt) {
            ...
            double E2 = 1/2.*blf_N0.InnerProduct(z_avg,z_avg);
            double E1 = 1/2.*blf_M0.InnerProduct(w_avg,w_avg);
            double D = -Re_inv*C0.InnerProduct(w_avg,z_old)
                       -Re_inv/2*C0T->InnerProduct(z_avg,w)
                       -Re_inv/2*C0T->InnerProduct(z_avg_old,w_old); 
