...
void f(const mfem::Vector &x, mfem::Vector &v); // TODO timedependence
...
int main(int argc, char *argv[]) {
    ...
    mfem::VectorFunctionCoefficient u_exact_coeff(dim, u_exact);
    ...
    *DT_n *= -1.;
    ...
    // mfem::Vector f_vec_1 (u.Size());
    b1.AddSubVector(f1,0); //TODO add f to rhs
    ...
    for (double t = dt ; t < tmax+dt ; t+=dt) {
        ...
        C_Re.AddMult(w,b2sub,-1);//TODO add f to rhs
        b2.AddSubVector(f1,0);
        ...
        b1.AddSubVector(b1sub,0);//TODO add f to rhs
    ...
    // convergence error
    double err_L2_u = u.ComputeL2Error(u_exact);
    std::cout << "L2err = "<< err_L2_u<<"\n";
...
void f(const mfem::Vector &x, mfem::Vector &returnvalue) { //TODO timedependence?
    ...
    returnvalue(0)=
        2 *pi* std::cos(2 *pi * (t + x(0) + x(1))) - std::cos(2 *pi * x(2)) + 
        4 *pi*pi* (2 - t)* std::cos(2 *pi * x(2)) + 
        2 *pi* std::cos(2 *pi * x(0))* std::sin(2 *pi * x(0)) + 
        2 *pi* (1 - t)*(1 - t)* std::cos(2 *pi * x(0))* std::sin(2 *pi * x(0)) - 
        4 *pi* t* std::cos(2 *pi * x(0)) * std::sin(2 *pi * x(0)) + 
        2 *pi* t*t* std::cos(2 *pi * x(0))* std::sin(2 *pi * x(0)) + 
        4 *pi* std::sin(2 *pi * x(0)) *std::sin(2 *pi * x(2)) - 
        6 *pi* t* std::sin(2 *pi * x(0))* std::sin(2 *pi * x(2)) + 
        2 *pi* t*t* std::sin(2 *pi * x(0))* std::sin(2 *pi * x(2));
    returnvalue(1)=
        2 *pi * std::cos(2 *pi * (t + x(0) + x(1))) - 
        2 *pi * std::cos(2 *pi * x(2)) *std::sin(2 *pi * x(0)) + 
        2 *pi * t*t* std::cos(2 *pi * x(2)) *std::sin(2 *pi * x(0)) + std::sin(2 *pi * x(2)) + 
        4 *pi*pi* (1 + t) *std::sin(2 *pi * x(2));
    returnvalue(2)=
        -4 *pi * std::cos(2 *pi * x(0))* std::cos(2 *pi * x(2)) +
        6 *pi * t* std::cos(2 *pi * x(0)) *std::cos(2 *pi * x(2)) - 
        2 *pi * t*t* std::cos(2 *pi * x(0)) *std::cos(2 *pi * x(2)) - std::sin(2 *pi * x(0)) + 
        4 *pi*pi* (1 - t) *std::sin(2 *pi * x(0)) - 
        6 *pi * std::cos(2 *pi * x(2)) *std::sin(2 *pi * x(2)) + 
        12 *pi * t *std::cos(2 *pi * x(2))* std::sin(2 *pi * x(2)) + 
        1/2 *(-4 *pi * (2 - t)*(2-t) *std::cos(2 *pi * x(2))* std::sin(2 *pi * x(2)) + 
        4 *pi * (1 + t)*(1+t) *std::cos(2 *pi * x(2)) *std::sin(2 *pi * x(2)));
}
